<template>
  ...
  <AppLayout>
    <div class="my-5 mx-auto w-75 text-center">
      ...
      <div class="card w-auto w-75 p-3 my-5 mx-auto" v-for="item in portfolio.data" :key="item.id">
        <h3 class=" mt-5 mb-3"><i class="bi bi-bar-chart"></i> {{ item.title }} {{ item.id }} </h3>
        <div class="d-flex flex-wrap justify-content-around align-items-center my-4">
          ...
          <p>{{ item.anons }}</p>
               ...
               <p class="text-primary"><em>{{ date(item) }}</em></p>
        <div class="d-flex flex-wrap gap-3 justify-content-around align-items-center my-5">
          <Link :href="route('portfolio.edit', item.id)" class="btn btn-outline-secondary">{{
      ...
      <!--      <pagination class="mt-5" :links="portfolio.data.links" />-->
      <!--      <Link v-for="link in portfolio.links" :href="link.url" v-html="link.label"/>-->
